...
namespace mgn {
    namespace terrain {
        ...
        RouteBeginRenderer::RouteBeginRenderer(mgn::graphics::Renderer * renderer, mgnMdTerrainView * terrain_view,
            graphics::Shader * shader)
        : RoutePointRenderer(renderer, terrain_view, shader)
        {
            mColor.Set(0.0f, 1.0f, 0.0f);
        }
        RouteBeginRenderer::~RouteBeginRenderer()
        {

        }
        void RouteBeginRenderer::Update(mgnMdTerrainProvider * provider)
        {
            // TODO: do 1 fetch per second, not every frame
            static std::vector<mgnMdWorldPoint> route_points;
            route_points.clear();
            if (provider->fetchRouteBegin(route_points))
            {
                mExists = true;
